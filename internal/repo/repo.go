...
func Clone(URL string, branch string, username string, password string, timeout time.Duration) (*gogit.GoGit, error) {
	nURL, err := url.Parse(URL)
	if err != nil {
		return nil, err
	}
	gitAuth, err := transportForURL(nURL, username, password)
	if err != nil {
		return nil, err
	}
...
func CommitPush(g *gogit.GoGit, authorName string, authorEmail string, message string, destinationUrl string, timeout time.Duration) error {
