...
setup_tmp() {
    TMP_DIR=$(mktemp -d -t lizz-install.XXXXXXXXXX)
    TMP_METADATA="${TMP_DIR}/lizz.json"
    TMP_HASH="${TMP_DIR}/lizz.hash"
    TMP_BIN="${TMP_DIR}/lizz.tar.gz"
    cleanup() {
        local code=$?
        set +e
        trap - EXIT
...
get_release_version() {
    if [[ -n "${LIZZ_VERSION}" ]]; then
      SUFFIX_URL="tags/v${LIZZ_VERSION}"
    else
      SUFFIX_URL="latest"
    fi
    ...
    VERSION_LIZZ=$(grep '"tag_name":' "${TMP_METADATA}" | sed -E 's/.*"([^"]+)".*/\1/' | cut -c 2-)
    if [[ -n "${VERSION_LIZZ}" ]]; then
        info "Using ${VERSION_LIZZ} as release"
    else
...
download_hash() {
    HASH_URL="https://github.com/${GITHUB_REPO}/releases/download/v${VERSION_LIZZ}/lizz_${VERSION_LIZZ}_checksums.txt"

    info "Downloading hash ${HASH_URL}"
    download "${TMP_HASH}" "${HASH_URL}"
    HASH_EXPECTED=$(grep " lizz_${VERSION_LIZZ}_${OS}_${ARCH}.tar.gz$" "${TMP_HASH}")
    HASH_EXPECTED=${HASH_EXPECTED%%[[:blank:]]*}
}

# Download binary from Github URL
download_binary() {
    BIN_URL="https://github.com/${GITHUB_REPO}/releases/download/v${VERSION_LIZZ}/lizz_${VERSION_LIZZ}_${OS}_${ARCH}.tar.gz"
...
setup_binary() {
    chmod 755 "${TMP_BIN}"
    info "Installing lizz to ${BIN_DIR}/lizz"
    tar -xzof "${TMP_BIN}" -C "${TMP_DIR}"

    local CMD_MOVE="mv -f \"${TMP_DIR}/lizz\" \"${BIN_DIR}\""
    if [[ -w "${BIN_DIR}" ]]; then
        eval "${CMD_MOVE}"
    else
        eval "sudo ${CMD_MOVE}"
